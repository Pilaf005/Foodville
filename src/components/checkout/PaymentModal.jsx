      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Recommended</p>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <PayRow id="gpay"      name="Google Pay UPI"  icon={<GPayIcon />}      selected={localSelected==="gpay"}      onSelect={handleSelect} />
              <PayRow id="phonepe"   name="PhonePe UPI"     icon={<PhonePeIcon />}   selected={localSelected==="phonepe"}   onSelect={handleSelect} />
              <PayRow id="amazonpay" name="Amazon Pay UPI"  icon={<AmazonPayIcon />} selected={localSelected==="amazonpay"} onSelect={handleSelect} noDivider />
            </div>
          </div>

          {/* CARDS */}
          <div className="space-y-2">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Cards</p>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <PayRow id="card" name="Add credit or debit card" icon={<CardIcon />} selected={localSelected==="card"} onSelect={handleSelect} actionText="ADD" noDivider />
            </div>
          </div>

          {/* NETBANKING */}
          <div className="space-y-2">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Netbanking</p>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <PayRow id="netbanking" name="Netbanking" icon={<NetbankingIcon />} selected={localSelected==="netbanking"} onSelect={handleSelect} actionText="ADD" noDivider />
            </div>
          </div>

          {/* PAY ON DELIVERY */}
          <div className="space-y-2">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Pay On Delivery</p>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <PayRow id="cod" name="Cash on Delivery" icon={<CodIcon />} selected={localSelected==="cod"} onSelect={handleSelect} noDivider />
            </div>
          </div>
        </div>
      </div>
