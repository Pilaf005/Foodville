...
export default function ProductGallery({ images = [], name = "" }) {
  ...
  const handleMouseMove = (e) => {
    ...
    // Bounds checking to lock the tracking lens within 20% to 80% boundaries (to keep it centered)
    const boundedX = Math.max(20, Math.min(80, x));
    const boundedY = Math.max(20, Math.min(80, y));

    setZoomPos({ x, y, boundedX, boundedY });
    setIsHovered(true);
  };
  ...
  const handleDoubleTap = () => {
    setMobileZoom((prev) => !prev);
  };

  // Magnified background position mapping calculations
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="absolute inset-0 rounded-3xl overflow-hidden">
          <img
            ...
            onError={(e) => {
              ...
              e.currentTarget.src = "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='400' height='400'><rect width='400' height='400' fill='%23F5F5F4'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' font-family='system-ui' font-size='18' fill='%23A8A29E'>No Image</text></svg>";
            }}
          />

          {/* Semi-transparent tracking lens overlay on top of main image */}
          {isHovered && (
            <div 
              ...
              style={{
                width: "40%",
                height: "40%",
                left: `${zoomPos.boundedX - 20}%`,
                top: `${zoomPos.boundedY - 20}%`,
              }}
            />
          )}
          ...
          {!isHovered && !mobileZoom && imgs.length > 1 && (
            <>
              <button
                onClick={prev}
                className="absolute left-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/90 shadow flex items-center justify-center text-ink hover:bg-white transition z-10"
              ...
              <button
                onClick={next}
                className="absolute right-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/90 shadow flex items-center justify-center text-ink hover:bg-white transition z-10"
        ...
        {/* ── Flipkart-style Zoomed Preview Box (appears to the right of main image) ── */}
        {isHovered && (
          <div 
          ...
          {imgs.map((src, i) => (
            <button
              ...
              className={`shrink-0 h-16 w-16 sm:h-20 sm:w-20 rounded-xl overflow-hidden border-2 transition-all duration-200 ${
                i === active
                  ? "border-olive shadow-md scale-105"
                  : "border-cardline hover:border-olive/50 opacity-70 hover:opacity-100"
              }`}
            ...
            >
              <img
                ...
                onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='80' height='80'><rect width='80' height='80' fill='%23F5F5F4'/></svg>"; }}
              />
            </button>
          ))}
        </div>
